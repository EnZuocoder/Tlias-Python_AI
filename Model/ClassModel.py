from datetime import datetime
from sqlalchemy import String, DateTime, func, Integer
from sqlalchemy.orm import Mapped, mapped_column
from Model.Base import Base
#班级模型类
class ClassModel(Base):
    __tablename__ = "clazz"  # 数据库表名
    id: Mapped[int] = mapped_column(primary_key=True, autoincrement=True)
    name:Mapped[str] = mapped_column(String(50),nullable=False)
    room:Mapped[str] = mapped_column(String(20))
    beginDate:Mapped[datetime] = mapped_column(DateTime,name="begin_date")
    endDate:Mapped[datetime] = mapped_column(DateTime,name="end_date")
    masterId:Mapped[int] = mapped_column(Integer,name="master_id")
    subject:Mapped[int] = mapped_column(Integer)
    # 创建时间
    createTime: Mapped[datetime] = mapped_column(DateTime, default=func.now(),name="create_time")
    # 更新时间
    updateTime: Mapped[datetime] = mapped_column(DateTime, default=func.now(), onupdate=func.now(),name="update_time")

